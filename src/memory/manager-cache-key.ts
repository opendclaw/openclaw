...
export function computeMemoryManagerCacheKey(params: {
  ...
  /** Optional session key — when this is a group session, the group ID is included in the cache key. */
  sessionKey?: string;
}): string {
  ...
  // Include group ID in cache key for group sessions to prevent cross-group memory pollution.
  const groupId = extractGroupIdFromSessionKey(params.sessionKey);
  if (groupId) {
    return `${params.agentId}:${groupId}:${params.workspaceDir}:${fingerprint}`;
  }
